import torch
from util import ensure_tensor_and_batched, ensure_tensor_and_batched_for_all, get_device, assert_same_length
...
def _is_same_side_of_plane(p_1s, p_2s, p_3s, ps_remaining, points, epsilon=0):
    ...
    # on mps normal_view is float64, so we need to convert it to float32
    if normal_view.dtype == torch.float64:
        normal_view = normal_view.to(torch.float32)
    if px_view.dtype == torch.float64:
        px_view = px_view.to(torch.float32)
